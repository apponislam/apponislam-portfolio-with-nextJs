...
// MultiSelect component (same as create)
...
interface BlogPostUpdateFormProps {
    authorId: string;
    blogData: z.infer<typeof formSchema> & { _id: string };
}

export function BlogPostUpdateForm({ authorId, blogData }: BlogPostUpdateFormProps) {
    ...
    // Set form values when blogData changes
    useEffect(() => {
        if (blogData) {
            form.reset({
                ...
                contentDetails: blogData.contentDetails,
                sections: blogData.sections,
                externalLinks: blogData.externalLinks || [],
                repositoryUrl: blogData.repositoryUrl || "",
            });
        }
    }, [blogData, form]);
    ...
    //     setIsLoading(true);
    //     try {
    //         const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/blog/${blogData._id}`, {
    //             method: "PATCH",
    //             headers: {
    ...
    return (
        <Card className="w-full max-w-4xl mx-auto">
            <CardHeader>
                ...
                <CardDescription>Edit the fields below to update your blog post.</CardDescription>
            </CardHeader>
            <CardContent>
                <Form {...form}>
                                        ...
                                        <div className="flex items-center gap-4">
                                            {field.value && (
                                                <div className="relative h-20 w-20">
                                                    <Image src={field.value} alt="Cover preview" fill className="object-cover rounded-md" unoptimized />
                                                </div>
                                            )}
                                            <FormControl>
